...
from paddle_models.Downstream_Lip import DownstreamModel
...
def test_save(args):
    ...
    # model.set_state_dict(paddle.load('finetune_save/949_FDA_model.pdparams'))
...
def train(
    ...
    for atom_bond_graphs, bond_angle_graphs, labels in data_gen:
        ...
        preds,_ = model(atom_bond_graphs, bond_angle_graphs)
...
def evaluate(
    ...
    for atom_bond_graphs, bond_angle_graphs, labels in data_gen:
        ...
        scaled_preds, g_repr = model(atom_bond_graphs, bond_angle_graphs)
...
def get_dataset(dataset_name, task=None):
    ...
    poses = np.load(path + 'conformations.npy')
...
def clear_main(args, dataset):
    ...
    compound_encoder = GeoGNNModel_lulu(compound_encoder_config)
    ...
    # if args.scaffold:
    #     train_dataset, valid_dataset, test_dataset = get_dataset(dataset_name, '1')
    ...
    # else:
    #     dataset = get_dataset(dataset_name, None)
    ...
    for epoch_id in range(args.max_epoch):
        ...
        if epoch_id == 100:
            encoder_opt.set_lr(0.0005)
            head_opt.set_lr(0.0005)
        # train_loss.backward()
        ...
        if val_metric < val_rmse:
            ...
            #     paddle.save(compound_encoder.state_dict(), './LipResult/' + str(args.seed) + '/RMSE_' + str(round(test_rmse,3))[2:] + '_Lip_encoder.pdparams')
            #     paddle.save(model.state_dict(), './LipResult/' + str(args.seed) + '/RMSE_' + str(round(test_rmse,3))[2:] + '_Lip_model.pdparams')
        else:
            patient += 1
    ...
    f.write('dataset: ' + args.dataset_name + ' init_model:' + str(args.init_model)+' seed:' + str(args.seed) +' rmse:' + str(round(test_rmse, 3)) + ' personr:' + str(round(test_pearson,3)) + '\n')
    f.close()
