...
def test_save(args):
    ...
    if dataset_name == 'LC50DM':
        ...
        if args.ratio == 0.9:
            init_encoder = 'downstream_save/IGC50_9_encoder.pdparams'
            init_model = 'downstream_save/IGC50_9_model.pdparams'
...
def clear_main(args, gne=False):
    ...
    if not gne:
        if args.dataset_name == 'LC50DM':
            args.ratio = 0.33
        elif args.dataset_name in ['IGC50', 'logP']:
            args.ratio = 0.5
        elif args.dataset_name == 'LD50':
            args.ratio = 0.7
        compound_encoder = vdWGraph(compound_encoder_config)
...
def main():
    ...
    Line 331-335, you can choose the MT-DNN tasks.
    """

    if args.re_train == 1:
        # ratios = [0.1, 0.15, 0.25, 0.3, 0.5]
        if args.gne == 0:
            ratios = [args.ratio]
            for ra in ratios:
                args.ratio = ra
